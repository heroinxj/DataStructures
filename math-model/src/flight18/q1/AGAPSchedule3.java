 ...
 */
public class AGAPSchedule3 extends IloCplex  {
    ...
    public AGAPSchedule3(Data data) throws IloException {
        this.data = data;
    }
...
//        Scanner cin = new Scanner(new BufferedReader(new FileReader("D://data//AGAP//flight.csv")));
        ...
        Scanner cin = new Scanner(new BufferedReader(new FileReader("E:\\2019学年\\1_数学建模2019\\练习题\\第十五届“华为杯”中国研究生数学建模竞赛—F题\\java-demo.csv")));
        //读取一行
        line = cin.nextLine();
        ...
        while(cin.hasNextLine()){
            ...
            flight.name = substr[1];
            if(substr[3].charAt(0) == '-'){
                String number = substr[3].substring(1);
                flight.startTime = -1 * Integer.parseInt(number);
            }else {
                flight.startTime = Integer.parseInt(substr[3]);
            }
            flight.arriveType = Integer.parseInt(substr[6]);
            flight.endTime = Integer.parseInt(substr[9]);
            flight.leaveType = Integer.parseInt(substr[12]);
            flight.planeType = substr[7].charAt(0);
        ...
        cin = new Scanner(new BufferedReader(new FileReader("E:\\2019学年\\1_数学建模2019\\练习题\\第十五届“华为杯”中国研究生数学建模竞赛—F题\\java-demo.csv")));
        cin.nextLine();
        while(cin.hasNextLine()){
            line = cin.nextLine();
            substr = line.split((","));
            Deport deport = new Deport();
            deport.name = substr[1];
    ...
    public static void main(String[] args) throws IloException, FileNotFoundException {
        ...
        AGAPSchedule3 cplex = new AGAPSchedule3(data);
